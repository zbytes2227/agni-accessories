...
<p className="text-center font-semibold text-xl mt-5 block sm:hidden">All Customer</p>
          {filteredCustomers.length > 0 ? (<div>
            <table className="w-full mt-3 text-sm text-left rtl:text-right text-gray-500 hidden md:block">
            ...
            {filteredCustomers.map((customer) => (
              <>
                <a href="#" class="block my-1.5 p-6 bg-white border border-gray-200 rounded-lg shadow hover:bg-gray-100">

                  <h5 class="mb-2 text-xl font-bold tracking-tight text-gray-900">{customer.CustomerName}

                    <kbd class="px-2 ms-2 py-1.5 text-xs font-semibold text-gray-800 bg-gray-100 border border-gray-200 rounded-lg">Id: {customer.CustomerID}</kbd>

                  </h5>

                  <p className="pb-1">Phone: {customer.CustomerPhone}</p>
                  <p className="pb-1">Email: {customer.CustomerEmail}</p>
                  <div className="flex mt-2">
                    <a  href={`customers/edit?id=${customer.CustomerID}`} type="button" class="px-3 py-2 text-xs font-medium text-center text-white bg-blue-700 rounded-lg hover:bg-blue-800 focus:ring-4 focus:outline-none focus:ring-blue-300 dark:bg-blue-600 dark:hover:bg-blue-700 dark:focus:ring-blue-800">Edit</a>
                    <button onClick={() => (deleteMe(customer.CustomerID))} type="button" class="px-3 mx-3 py-2 text-xs font-medium text-center text-white bg-red-700 rounded-lg hover:bg-red-800 focus:ring-4 focus:outline-none focus:ring-blue-300">Delete</button>
                  </div>
